...
def save_global_results(dicc_results_test, args):
    ...
    for i in range(args.num_tasks):
        for j in range(args.num_tasks):
            list_tasks_so_far = [k for k in range(1,i+2)]
            worksheet.write(cont_tasks, 0, f"Test accuracy on task {j+1} after task {list_tasks_so_far}")
            cont_tasks += 1
        worksheet.write(cont_avg_task, 0, f"Test average accuracy after task {list_tasks_so_far}")
