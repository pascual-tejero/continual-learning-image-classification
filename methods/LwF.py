...
def lwf_training(datasets, args):
    ...
    for i in range(len(datasets)):
        if os.path.exists(f'./models/lwf_{args.dataset}_task_{i+1}.pth'):
            os.remove(f'./models/lwf_{args.dataset}_task_{i+1}.pth')
    ...
    for id_task_dataset, task in enumerate(datasets):
        ...
        for epoch in range(args.epochs):
            ...
            train_loss_epoch = train_epoch(model, device, train_loader, optimizer, id_task_dataset, epoch, args)
...
def train_epoch(model, device, train_loader, optimizer, id_task_dataset, epoch, args):
    ...
    for images, labels in train_loader:
        ...
        if id_task_dataset+1 > 1:  # Apply LwF from the second task onward
            ...
            distillation_loss = args.lwf_lambda * F.kl_div(F.log_softmax(outputs, dim=1), F.softmax(prev_outputs, dim=1)) 
            train_loss += distillation_loss # Add the distillation loss to the training loss

        train_loss_acc += train_loss.item() # Accumulate the training loss
        train_loss.backward() # Backward pass
        optimizer.step() # Optimize

    train_loss_epoch = train_loss_acc/len(train_loader)
